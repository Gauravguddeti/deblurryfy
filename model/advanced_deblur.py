...
class AdvancedDeblurModel:
    ...
    def wiener_deconvolution(self, image_array, kernel_size=3):
        ...
        kernel = kernel / np.sum(kernel)        # Apply improved Richardson-Lucy deconvolution to each channel
        result = np.zeros_like(image_array)
        
        for c in range(3):
            channel = image_array[:, :, c]
            # Use moderate iterations for good sharpness without overflow
            deconv_result = restoration.richardson_lucy(channel, kernel, num_iter=25)
            # Apply gentle clipping to remove border artifacts
            result[:, :, c] = np.clip(deconv_result, 0, 1)
        
        return result
    def unsharp_mask_advanced(self, image_array, radius=1.2, amount=2.8):
        """Advanced unsharp masking with border artifact reduction"""
        result = np.zeros_like(image_array)
        ...
        for c in range(3):
            ...
            # Create mask for controlled sharpening
            mask = channel - blurred
            # Apply moderate sharpening
            sharpened = channel + amount * mask
            # Apply border smoothing to reduce artifacts
            result[:, :, c] = self.reduce_border_artifacts(sharpened)
    ...
    def multi_scale_sharpening(self, image_array):
        """Apply controlled multi-scale sharpening for crisp results"""
        # More conservative sharpening to prevent artifacts
        scales = [0.6, 1.0, 1.4]
        weights = [0.2, 0.6, 0.2]
        ...
        for scale, weight in zip(scales, weights):
            # Apply unsharp mask at this scale with moderate sharpening
            sharpened = self.unsharp_mask_advanced(image_array, radius=scale, amount=3.0)
    ...
    def deblur_image(self, input_image):
        ...
        if characteristics['is_test_pattern']:
            # For test images: use optimized pipeline for good sharpness
            print("Applying optimized deblurring for test pattern...")
              # Step 1: Controlled Wiener deconvolution
            deblurred = self.wiener_deconvolution(img_array, kernel_size=3)
            
            # Step 2: Multi-scale sharpening with moderate intensity
            deblurred = self.multi_scale_sharpening(deblurred)
            
            # Step 3: Final edge enhancement with slightly increased sharpening
            deblurred = self.unsharp_mask_advanced(deblurred, radius=0.7, amount=3.6)
            
            # Step 4: Apply bilateral filter to reduce artifacts while preserving edges
            deblurred_uint8 = (deblurred * 255).astype(np.uint8)
            for c in range(3):
                deblurred_uint8[:, :, c] = cv2.bilateralFilter(
                    deblurred_uint8[:, :, c], d=4, sigmaColor=10, sigmaSpace=10
                )
        ...
        # Final enhancement with conservative gamma correction
        deblurred = exposure.adjust_gamma(deblurred, gamma=0.94)
        ...
        if result_image.size != original_size:
          ...
          # Apply final sharpening filter with slightly increased settings
        result_image = result_image.filter(
            ImageFilter.UnsharpMask(radius=0.7, percent=190, threshold=0)
        )
        return result_image
